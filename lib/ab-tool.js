...
Options are:
    ...
    -s timeout      Maximum number of seconds to wait before the socket  times  out.
...
AB.prototype.spawnProcess = function (callback) {
    ...
    proc.on('exit', function (data){
        if (data == 119) {
            self.emit('error', {iteration: iteration, code: data, message: 'The timeout specified has expired', args: args});
            return;
        }
        if(data > 0) {
